 ...
 */
		...
		animate : function() {
			...
			function zoomIn() {
				...
				TweenMax.to([arrow_left, arrow_leftOverlay], 0.5, {opacity: 1, display:'block', left:'-24px', ease: Strong.easeInOut, delay:0.1}); // slide in
				TweenMax.to([arrow_right, arrow_rightOverlay], 0.5, {opacity: 1, display:'block', right:'-24px', ease: Strong.easeInOut, delay:0.1}); // slide in
			...
			function moveCarousel() {
				var carouselItems = frame2.childNodes,
					...
					tl 			  = new TimelineMax(),
					id 			  = 1;
				TweenMax.set(carouselItems[1],{left:Ad.WIDTH,opacity:1,top:-600});
				...
				if( (carouselItems.length >2)){
					holdTime+= 1.85;
					TweenMax.set(carouselItems[2],{left:Ad.WIDTH,opacity:1,top:-600});
			...
			function animateEndFrame () {
				...
				//ARROWS
				setTimeout(function(){
					TweenMax.to(arrow_left,.35,{left:"0px"});
					TweenMax.to(arrow_right,.35,{right:"0px"});	
				}, 500)
				...
				TweenMax.to([frame5_copyImg], 0.5, {opacity: 1, ease: Circ.easeInOut, delay:.25});
				...
				TweenMax.to(carousel, 0.5, {opacity: 1, ease: Strong.easeOut, delay:.05,onComplete:function () {
					if(products_info.length >=2){
						...
						TweenMax.to(slide2, 0.5, {left: 75, ease: Strong.easeInOut, delay:.15});
					}
					if(products_info.length >=3){
						slide3 = products_info[2].slide;
						TweenMax.set(slide3,{opacity:.7});
						TweenMax.to(slide3, 0.5, {left: -80, ease: Strong.easeInOut, delay:.15});
					}
					if(products_info.length >=4){
						slide4 = products_info[3].slide;
						TweenMax.set(slide4,{opacity:1});
						TweenMax.to(slide4, 0.5, {left: 0, ease: Strong.easeInOut, delay:.15});
					}
				}});
		...
		setUpCarousel : function(){
			...
			for(var i = 0; i < products_info.length; i++){
				...
				if(i == 1){
					(products_info[i].slide).style.left = "160px";
					(products_info[i].slide).style.opacity = .0;
					TweenMax.set((products_info[i].imageHold),{scaleX:.35,scaleY:.35});
					TweenMax.set((products_info[i].imageHold),{scaleX:.35,scaleY:.35});
				}
				if(i == 2){
					(products_info[i].slide).style.left = "-160px";
		...
		resetCarousel : function(e){
			...
			//ARROWS
			TweenMax.to(arrow_left,.35,{left:"-=25px"});
			TweenMax.to(arrow_right,.35,{right:"-=25px"});
	...
	Feed = {
		...
		createItem : function(data, num,sku) {
			...
			if(data){
				...
				TweenMax.set(wrapper,{transformOrigin:'right bottom',top:-600});
